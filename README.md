...
# Squeezelite-esp32
NOTE# This is just a fork of the popular Squeezelite-ESP32 for the ESP32-S3-WROOM-1-N16R8 to ease in upgrades.  All of the credit goes to sle118 and crew.  I'm just modifying a few bits to streamline updates on the N16R8 module.  After running into an issue with one of my home audio prototypes, and having to resolder a controller for firmware flashing, I figured I'd try to make this easier for myself and create an S3-N16R8 friendly repo.  I'll do my best to keep up with sle118 and crew on commits for major releases.  The N16R8 is my controller of choice, so when there is an issue needing an update I'll be highly motivated to spend some time at it.  I hope this helps someone be lazy and save a few bucks by using the N16R8. Bluetooth seems to still break the compile, so it is disabled in the sdkconfig.  Build based on Espressif IDF v4.4.8.

The rest of sle118's README below (unmodified):
## Forewords
